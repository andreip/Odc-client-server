...
public class BaseClient implements Runnable {
	...
	protected Map<SocketChannel, BaseRspHandler> rspHandlers = Collections.synchronizedMap(new HashMap<SocketChannel, BaseRspHandler>());

	public BaseClient() {}
	public BaseClient(InetAddress hostAddress, int port) throws IOException {
		this.hostAddress = hostAddress;
		this.port = port;
		this.selector = this.initSelector();
	}

	public void send(byte[] data, BaseRspHandler handler) throws IOException {
...
//				logger.debug("Selector selected something.");
				...
				displayError("Connection closed by other user! Communication canceled.");
			}
		}
	}
	...
	protected void read(SelectionKey key) throws IOException {
		...
		try {
			...
			key.cancel();
			socketChannel.close();
			displayError("Unable to read from user! Connection canceled.");
			return;
		}
	...
	protected void handleResponse(SocketChannel socketChannel, byte[] data, int numRead) throws IOException {
		...
		BaseRspHandler handler = (BaseRspHandler) this.rspHandlers.get(socketChannel);
		...
		synchronized (this.pendingData) {
			...
			while (!queue.isEmpty()) {
				...
				try {
					...
					displayError("Unable to write to user! Connection canceled.");
				}
